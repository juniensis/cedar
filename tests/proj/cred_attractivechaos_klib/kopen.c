...
static int http_open(const char *fn) {
  ...
  if (proxy == 0) {
    ...
    host =
        (strstr(proxy, "http://") == proxy) ? strdup(proxy + 7) : strdup(proxy);
    for (q = host; *q && *q != ':'; ++q)
      ;
    if (*q == ':')
      *q++ = 0;
...
static int kftp_get_response(ftpaux_t *aux) {
  ...
  while (read(aux->ctrl_fd, &c,
    ...
    aux->response[n++] = c;
    if (c == '\n') {
      if (n >= 4 && isdigit(aux->response[0]) && isdigit(aux->response[1]) &&
          isdigit(aux->response[2]) && aux->response[3] != '-')
        break;
      n = 0;
      continue;
    }
  ...
  if (strstr(fn, "http://") == fn) {
    ...
    if (*p == '<') { // pipe open
      ...
      if (pid == 0) { /* the child process */
        char *
            *argv; /* FIXME: I do not know if this will lead to a memory leak */
        close(pfd[0]);
